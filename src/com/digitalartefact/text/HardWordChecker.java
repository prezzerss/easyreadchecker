...
public class HardWordChecker 
{
    ...
    private static String buildSuggestion(String input, List<String> defsOut) 
    {
        ...
        for(Term t : hardWords) 
        {
            Pattern pattern = Pattern.compile("(?i)\\b" + Pattern.quote(t.hardWord) + "\\b");
            ...
            while(matcher.find()) 
            {
                if(t.simpler != null) 
                {
                    matcher.appendReplacement(stringBuilder, Matcher.quoteReplacement(t.simpler));
                } 
                else 
                {
                    String originalMatched = matcher.group();
                    String upper = originalMatched.toUpperCase();
                    matcher.appendReplacement(stringBuilder, Matcher.quoteReplacement(upper));
